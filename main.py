...
class ConfigManager:
    ...
    def save(self) -> None:
        ...
        if self.loc_config and self.app_settings:
            data: dict[str, dict[str, str | int]] = {"location_config": asdict(self.loc_config), "app_settings": asdict(self.app_settings)}
...
class BulbController:
    def __init__(self, ip: str, state_change_callback_recver: Callable[[str], None] | None = None) -> None:
...
class App(tk.Tk):
    ...
    def create_widgets(self) -> None:
        ...
        # Toggle button
        ttk.Button(self.mainframe, text="Toggle", command=self.bulb.toggle).grid(column=1, row=1)
        ...
        # Dialog window
        if self.ip is None:
            messagebox.showerror(message="Missing IP address. Please enter a valid IP in settings.")
    ...
    def _init_external(self) -> None:
        ...
        self.ip = BULB_IP
        if self.ip:
            self.bulb = BulbController(str(self.ip), self.set_bulb_state)
        # notify user and ask to enter IP
        # we can't show here dialog window, because create_widgets() was not yet called
        # do nothing and show dialog in create_widgets() if no ip? >> ip must be set with self then
