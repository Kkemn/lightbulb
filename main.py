...
class Application(Tk):
    ...
    def create_widgets(self) -> None:
        ...
        # self.auto_on_var = IntVar(value=0)
        ...
        # self.auto_off_var = IntVar(value=0)
        ...
        ttk.Button(self.mainframe, text="Save config", command=self.save_config).grid(column=2, row=10)
    ...
    def set_location(self) -> None:
        payload: dict[str, str | int] = {"name": self.user_input_loc.get(), "count": 1}
        response: requests.Response = requests.get(url=GEOCODING_URL, params=payload)
